...
<template>
  <div class="min-h-screen bg-white pb-8">
    <div class="max-w-lg mx-auto pt-8 px-4 sm:px-6 lg:px-8">
      <h1 class="text-4xl font-bold text-black mb-6">Weather</h1>
      ...
      <div v-if="searchQuery && filteredSearchResults.length > 0" class="mb-6 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
        <div 
          v-for="location in filteredSearchResults" 
          :key="location.id"
      ...
      <!-- Saved Locations -->
      <div v-if="savedLocations.length > 0" class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-xl font-bold text-gray-900">Saved Locations</h2>
              <p class="text-sm text-gray-500">Your favorite weather locations</p>
            </div>
          </div>
          <div v-if="isUpdating" class="flex items-center gap-2 text-sm text-gray-500 bg-blue-50 px-3 py-1 rounded-full">
        ...
        <div v-for="location in savedLocations" :key="location.id" class="mb-4">
          <!-- Loading state -->
          <div v-if="!location.weatherData" class="rounded-2xl overflow-hidden shadow flex items-center min-h-[110px] relative bg-gradient-to-r from-blue-100 to-blue-300">
            ...
            <!-- Retry button -->
            <button 
              @click="() => fetchWeatherForSavedLocation(location)"
          ...
          >
            <!-- Remove button -->
            <button 
              @click.stop="removeFromSaved(location.id)"
            ...
            <span class="absolute top-3 right-12 text-xs text-gray-500 bg-gray-100 rounded px-2 py-0.5 font-mono">
              {{ location.weatherData.location.country }}
            </span>
            
            <div class="flex-1 px-5 py-4 flex flex-col justify-between">
              <div class="flex flex-row items-center gap-2">
                <span class="text-xl font-bold text-gray-900">{{ location.weatherData.location.displayName }}</span>
                <span class="text-xs text-gray-500">{{ location.weatherData.location.region }}</span>
              </div>
                <div class="flex flex-row items-center gap-2">
                  <span class="text-xs text-gray-500">Last updated: {{ formatTime(location.weatherData.current.last_updated) }}</span>
                </div>
              <div class="flex items-center gap-2 mt-2">
                <img 
                  v-if="location.weatherData.current.condition.icon" 
                  :src="location.weatherData.current.condition.icon" 
                  :alt="location.weatherData.current.condition.text"
                  class="w-10 h-10"
                />
                <span class="text-gray-500 text-sm">{{ location.weatherData.current.condition.text }}</span>
              </div>
            </div>
            <div class="flex flex-col items-end justify-center h-full pr-6 py-4 gap-1 mt-5">
              <div class="flex flex-row items-center gap-2">
                <span class="text-3xl font-bold text-gray-900">{{ location.weatherData.current.temp_c || '--' }}<span class="align-super text-xl">°</span></span>
              </div>
              <div class="flex flex-row items-center gap-2">
                <span class="text-xs text-gray-500">{{ location.weatherData.forecast[0]?.day.mintemp_c || '--' }}° - {{ location.weatherData.forecast[0]?.day.maxtemp_c || '--' }}°</span>
                <span class="text-xs text-gray-500">Feels like {{ location.weatherData.current.feelslike_c || '--' }}°</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="selectedLocation && !savedLocations.find(loc => loc.id === selectedLocation.id)">
          ...
          <div class="mb-6">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 class="text-xl font-bold text-gray-900">Search Result</h2>
                <p class="text-sm text-gray-500">Weather information for your search</p>
              </div>
            </div>
          </div>
        <!-- Loading state -->
        <div v-if="isWeatherLoading" class="mb-4 rounded-2xl overflow-hidden shadow flex items-center min-h-[110px] relative bg-gradient-to-r from-blue-100 to-blue-300">
          <div class="flex-1 px-5 py-4 flex items-center justify-center">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
        ...
        <div 
          ...
          class="mb-4 rounded-2xl overflow-hidden shadow flex items-center min-h-[110px] relative" 
          style="cursor:pointer;"
          @click="openDialog"
        >
          <!-- Save button -->
          <button 
            @click.stop="addToSaved(selectedLocation)"
          ...
          <div class="flex-1 px-5 py-4 flex flex-col justify-between">
            ...
            <span class="text-xs text-gray-500 mt-1">Last updated: {{ formatTime(weatherData.current.last_updated) }}</span>
            <div class="flex items-center gap-2 mt-2">
              <img 
          ...
          <div class="flex flex-col items-end justify-center h-full pr-6 py-4 gap-1 mt-5">
            <div class="flex flex-row items-center gap-2">
              <span class="text-3xl font-bold text-gray-900">{{ weatherData.current.temp_c || '--' }}<span class="align-super text-xl">°</span></span>
            </div>
            <div class="flex flex-row items-center gap-2">
              <span class="text-xs text-gray-500">{{ weatherData.forecast[0]?.day.mintemp_c || '--' }}° - {{ weatherData.forecast[0]?.day.maxtemp_c || '--' }}°</span>
              <span class="text-xs text-gray-500">Feels like {{ weatherData.current.feelslike_c || '--' }}°</span>
            </div>
          </div>
        </div>
